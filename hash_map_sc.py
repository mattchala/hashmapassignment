...
# Description: HashMap class that uses the chaining method with LinkedLists.  Contains various methods including
#   put(), get(), remove(), resize(), get_keys(), etc.
...
class HashMap:
    ...
    def put(self, key: str, value: object) -> None:
        ...
        key argument already exists in the hash table.
        """
        index = self.hash_function(key) % self.capacity
        if not self.buckets[index].remove(key):
            self.size += 1
    ...
    def resize_table(self, new_capacity: int) -> None:
        ...
        # create linked list of all new values
        all_elements = LinkedList()
        for index in range(self.capacity):
            if self.buckets[index].length() > 0:
                for elem in self.buckets[index]:
                    all_elements.insert(elem.key, elem.value)

        # reset hash map values and change capacity
        self.buckets = DynamicArray()
        ...
        # add all elements back into hash map with new capacity
